...
/**
 * An object that can find its way through a maze, leaving behind a colored trail showing its path
 */
...
public class Monkey extends JPanel
{
    ...
    /**
     * The start and goal rooms are unstained. This method investigates all possible paths that begin at start and do
     * not go through any stained rooms. As it enters rooms on the path, it stains them. As it backs out, it unstains
     * them. At any given moment in time, then, the current path that the method is investigating is stained.
     * 
     * If the method reaches the goal, it returns true and leaves the path stained. Otherwise, it returns false and
     * erases all the stains that it made.
     * 
    ...
    public boolean doSearch (Room start, Room goal)
    {
        ...
        if (start.hasDoor("E"))
        {
            ...
            if (!r.isStained(color) && doSearch(r, goal))
                return true;
        }
        if (start.hasDoor("W"))
        {
